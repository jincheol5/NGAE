    ...
    def train(
        ...
        for epoch in tqdm(range(kwargs["epoch"]),desc=f"Model Training..."):
            ...
            for batch_data in tqdm(train_loader,desc=f"Training epoch {epoch+1}..."):
                ...
                pred_r_traj=pred_result["reachability"] # [B,max_bf_len-1,N]
                pred_d_traj=pred_result["distance"] # [B,max_bf_len-1,N]
                edge_score_traj=pred_result["edge_score"] # [E,max_bf_len-1,1]

                ### Loss
                reachability_loss=TrainUtils.compute_reachability_loss(
                    r_traj=r_traj,
                ...
                loss=reachability_loss+distance_loss+predecessor_loss
            ...
            Validate model
            """
            val_result=BothTrainer.validate(
                model=model,
                val_loader=val_loader,
                **kwargs
            )
            val_r_acc=val_result["r_acc"]
            val_p_acc=val_result["p_acc"]
            print(f"Validate Reachability ACC: {val_r_acc}")
            print(f"Validate Predecessor ACC: {val_p_acc}")
    ...
    def validate(
        ...
        with torch.no_grad():
            for batch_data in tqdm(val_loader,desc=f"Validate..."):
                ...
                ### Accuracy
                reachability_acc=Metric.compute_reachability_accuracy(
                    r_traj=r_traj,
                    pred_r_traj=pred_r_traj,
                    bfs_mask=bfs_mask
                )
                predecessor_acc=Metric.compute_predecessor_accuracy(
                    p_traj=p_traj,
                    edge_score_traj=edge_score_traj,
                    edge_index=edge_index,
                    bf_mask=bf_mask
                )
                r_acc_list.append(reachability_acc)
                p_acc_list.append(predecessor_acc)
        return {
            "loss":torch.stack(loss_list).mean().item(),
            "r_acc":sum(r_acc_list)/len(r_acc_list),
            "p_acc":sum(p_acc_list)/len(p_acc_list),
        }
